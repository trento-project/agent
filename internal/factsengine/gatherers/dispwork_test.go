...
func (suite *DispWorkGathererTestSuite) TestDispWorkGatheringSuccess() {
	...
	suite.mockExecutor.
		...
		On("OutputContext", mock.Anything, "/usr/bin/su", "-", "qasadm", "-c", "\"disp+work\"").
		Return(partialOutput, nil).
		On("OutputContext", mock.Anything, "/usr/bin/su", "-", "qa2adm", "-c", "\"disp+work\"").
		Return(unsortedOutput, nil).
		On("OutputContext", mock.Anything, "/usr/bin/su", "-", "devadm", "-c", "\"disp+work\"").
