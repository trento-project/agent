...
func (suite *ParserTestSuite) TestParse() {
	...
	suite.True(data.Nodes[0].Online)
	suite.True(data.Nodes[0].ExpectedUp)
	suite.True(data.Nodes[0].DC)
	suite.False(data.Nodes[0].Unclean)
	suite.False(data.Nodes[0].Shutdown)
	suite.False(data.Nodes[0].StandbyOnFail)
	suite.False(data.Nodes[0].Maintenance)
	suite.False(data.Nodes[0].Pending)
	suite.False(data.Nodes[0].Standby)
	suite.Equal("node02", data.Nodes[1].Name)
	suite.Equal("1084783376", data.Nodes[1].ID)
	suite.True(data.Nodes[1].Online)
	suite.True(data.Nodes[1].ExpectedUp)
	suite.False(data.Nodes[1].DC)
	suite.False(data.Nodes[1].Unclean)
	suite.False(data.Nodes[1].Shutdown)
	suite.False(data.Nodes[1].StandbyOnFail)
	suite.False(data.Nodes[1].Maintenance)
	suite.False(data.Nodes[1].Pending)
	...
	suite.False(data.Resources[0].Active)
...
func (suite *ParserTestSuite) TestParseClones() {
	...
	suite.Len(data.Clones[0].Resources, 2)
	suite.Len(data.Clones[1].Resources, 2)
...
func (suite *ParserTestSuite) TestParseGroups() {
	...
	data, err := p.Parse()
	suite.Require().NoError(err)
	suite.Len(data.Groups, 2)

	suite.Equal("grp_HA1_ASCS00", data.Groups[0].ID)
	suite.False(data.Groups[0].Managed)
	suite.Len(data.Groups[0].Resources, 3)
	...
	suite.True(data.Groups[1].Managed)
	suite.Len(data.Groups[1].Resources, 3)
